        ...
        <div className="text-white px-6 py-4 my-2 font-inter">
            <a href={href} className="grid md:grid-cols-7 grid-cols-3 gap-3 transition-all group">
                ...
                <div className="md:col-span-6 col-span-2">
                    <h1 className="md:text-2xl text-lg font-bold group-hover:text-animemine">{title}</h1>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {
                            genres.map((genre, index) => (
                                <p key={index} className="mt-2 bg-animemine uppercase font-semibold px-2 py-[2px] text-xs">{genre.genre_name}</p>
                            ))
                        }
                    </div>
                </div>
